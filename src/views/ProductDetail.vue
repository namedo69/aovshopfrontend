...
const productCategoryId = computed(() => product.value?.category_id ?? product.value?.category?.id ?? null)

const loadRelated = async () => {
  relatedProducts.value = []
  if (!productCategoryId.value) return
  try {
    const res = await shopApi.getProducts({ category_id: productCategoryId.value, per_page: 5 })
...
// Reload when navigating between products (e.g. via related items)
