<template>
  <div class="navbar-container">
    <nav class="navbar" :class="{ 'navbar-scrolled': isScrolled }">
        <div class="container navbar-content">
          ...
          <form class="nav-search" @submit.prevent="submitSearch">
            <input
              v-model="searchText"
              type="text"
              placeholder="Tìm kiếm tài khoản, danh mục..."
              aria-label="Tìm kiếm sản phẩm"
            />
            <button type="submit" aria-label="Tìm kiếm">🔍<span class="nav-search-label">Tìm</span></button>
          </form>
          
          <!-- Desktop Actions -->
          <div class="navbar-actions desktop-actions">
            <!-- Theme Toggle -->
            <button class="theme-toggle icon-btn" @click="themeStore.toggle" :title="themeStore.isDark ? 'Chuyển sang sáng' : 'Chuyển sang tối'">
              {{ themeStore.isDark ? '☀︎' : '⏾' }}
            </button>
            ...
            <template v-if="authStore.isAuthenticated">
              ...
              <router-link to="/cart" class="cart-badge icon-btn" title="Giỏ hàng">
                🛒
                <span v-if="cartStore.itemCount" class="cart-count">{{ cartStore.itemCount }}</span>
              </router-link>

              <div class="dropdown">
                <button class="avatar-btn" @click="showDropdown = !showDropdown" :title="authStore.user?.name">
                  {{ (authStore.user?.name || 'K')[0].toUpperCase() }}
                </button>
          ...
          <!-- Mobile quick cart -->
          <router-link to="/cart" class="cart-badge icon-btn mobile-cart" title="Giỏ hàng">
            🛒
            <span v-if="cartStore.itemCount" class="cart-count">{{ cartStore.itemCount }}</span>
          </router-link>
        ...
        <!-- Category nav strip (template style: deep-red bar) -->
        <div class="container nav-strip-container">
          <ul class="nav-strip">
            <li><router-link to="/">Trang chủ</router-link></li>
            <li class="nav-dropdown">
              <button class="nav-strip-toggle" @click="showCategoryDropdown = !showCategoryDropdown">
                Danh mục ▾
              </button>
            ...
            <li><router-link to="/deposit">Nạp tiền</router-link></li>
...
const submitSearch = () => {
  ...
  router.push({ path: '/products', query: q ? { search: q } : {} })
}
...
@keyframes pulse-balance {
  0%, 100% { box-shadow: 0 0 0 0 rgba(184, 0, 0, 0.3); }
...
/* Round icon buttons (template style) */
.icon-btn {
  width: 40px;
  height: 40px;
  border-radius: 50%;
...
/* Red category nav strip (shopnhanviencu template style) */
.nav-strip-container {
  display: none;
}
...
@media (min-width: 769px) {
  ...
  /* Dropdown links keep dark text on the white menu */
  .nav-strip .nav-dropdown-menu a {
    color: var(--text);
  }

  .nav-strip .nav-dropdown-menu a:hover {
    background: var(--bg-tertiary);
    color: var(--primary);
  }

  .nav-strip .nav-dropdown {
    position: relative;
  }
}
